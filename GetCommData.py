...
class PyQt_kiwoomConnect:
    ...
    def OnReceiveTrData(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext, nDataLength, sErrorCode, sMessage,
                        sSplmMsg):
        if sRQName == "opt10001_req":
            name = self.kiwoom.dynamicCall("GetCommData(QString, QString, QString, QString",
                                           [sTrCode, "", 0, "종목명"]).strip()
            volume = self.kiwoom.dynamicCall("CommGetData(QString, QString, QString, int, QString)", sTrCode, "",
                                             sRQName, 0, "거래량").strip()
            code = self.kiwoom.dynamicCall("CommGetData(QString, QString, QString, int, QString)", sTrCode, "", sRQName,
                                           0, "종목코드").strip()
            print(f"종목명 : {name}")
            print(f"거래량 : {volume}")
            print(f"종목코드 : {code}")
